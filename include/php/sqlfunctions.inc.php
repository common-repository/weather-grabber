...
$endtime = time();
	...
	while ($row = mysqli_fetch_array($result)) {
				$weatherArray['SQLData']['rain'] = InchtoMM($row[0]);
				$i++;
			}
...
$endtime = time();
	...
	$i = 0;
	while ($row = mysqli_fetch_array($result)) {
				$weatherArray['SQLData']['ET'] = InchtoMM($row[0]);
				$i++;
			}
$result->close();
...
$endtime = time();
	...
	$i = 0;
	while ($row = mysqli_fetch_array($result)) {
				$weatherArray['SQLData']['rain24hr'] = InchtoMM($row[0]);
				$i++;
			}
...
SELECT max(high) FROM windSpeed WHERE dateTime BETWEEN " . $starttime . " AND " . $endtime . ") AND dateTime BETWEEN " . $starttime . " AND " . $endtime . ";";
	...
	$sql = "SELECT ROUND((high * 1.609344), 2), whenHigh, FROM_UNIXTIME(timeHigh" . $weatherArray['timeOffsetSign'] . $weatherArray['timeServerOffsetUnixNum'] . ",'%H:%i') FROM windGust WHERE high = (
...
$endtime = time();
	...
	$i = 0;
	while ($row = mysqli_fetch_array($result)) {
				$weatherArray['SQLData']['ET24hr'] = InchtoMM($row[0]);
				$i++;
			}
...
//	echo $sql;
	while ($row = mysqli_fetch_array($result)) {
				...
				{
				$weatherArray['SQLData']['ET24HourlySum']= array();
				$weatherArray['SQLData']['ET24HourlySum'][$i] = 0 - InchtoMM($row[0]);
				$weatherArray['SQLData']['ET24HourlyTime']= array();
				$weatherArray['SQLData']['ET24HourlyTime'][$i] = $row[1];	
				$weatherArray['SQLData']['ET24HourlyUnixTime']= array();
...
$sql = "SELECT ROUND(" . $weatherArray['archiveTableArray']['outTemp'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['inTemp'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['barometer'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['outHumidity'] . ",0),ROUND(" . $weatherArray['archiveTableArray']['inHumidity'] . ",2)," . $weatherArray['archiveTableArray']['rain'] . "," . $weatherArray['archiveTableArray']['rainRate'] . ",ROUND(" . $weatherArray['archiveTableArray']['windSpeed'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['windGust'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['windDir'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['windGustDir'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['dewpoint'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['windchill'] . ",2),ROUND(" . $weatherArray['archiveTableArray']['heatindex'] . ",2)," . $weatherArray['archiveTableArray']['dateTime'];
	...
	if ($weatherArray['sensors'] == 1) { //IN CASE THERE ARE EXTENDED SENSORS
	
	$sql = $sql . ",ROUND(" . $weatherArray['archiveTableArray']['radiation'] . ",1),ROUND(" . $weatherArray['archiveTableArray']['UV'] . ",1)," . $weatherArray['archiveTableArray']['ET'];

	}

$sql = $sql . " from " . $weatherArray['dbtableName'] . " ORDER BY ".$weatherArray['archiveTableArray']['dateTime']." DESC LIMIT 288;";

//END BUILDING SQL STATEMENT
	...
	while ($row = mysqli_fetch_array($result)) {
			$i--;
				...
				if ($weatherArray['sensors'] == 1) { //IN CASE THERE ARE EXTENDED SENSORS
				$weatherArray['SQLData']['SolarRadSQL']= array();
				$weatherArray['SQLData']['SolarRadSQL'][$i] = $row[15];
				$weatherArray['SQLData']['UVSQL'] = array();
				$weatherArray['SQLData']['UVSQL'][$i] = $row[16];
				$weatherArray['SQLData']['ET'] = array();
...
$sql = $sql . " from " . $weatherArray['dbtableName'] . " ORDER BY ".$weatherArray['archiveTableArray']['dateTime']." DESC LIMIT 2016;";
...
$sql = $sql . " from " . $weatherArray['dbtableName'] . " ORDER BY ".$weatherArray['archiveTableArray']['dateTime']." DESC LIMIT 8928;";
