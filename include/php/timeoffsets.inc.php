...
if ($input != NULL && $input!= 'NULL' && $input!= 'NA') {

if ((strpos($input,':') !== false) && ($selector == 0)) 
{ // No Change Time add Today/Yesterday for H:D format
...
$M = substr($input,4,2) * 1;
$d = substr($input,6,2) * 1;
$h = substr($input,9,2) * 1;
$m = substr($input,12,2) * 1;
...
function OFFSETtimes($weatherArray) {
		...
		$weatherArray['astroMarineRiseTime'] = timeconverter($weatherArray['astroMarineRiseTime'], 1, $weatherArray);
		...
		$weatherArray['astroMarineSetTime'] = timeconverter($weatherArray['astroMarineSetTime'], 1, $weatherArray);
	...
	//	echo $weatherArray['SQLData'][$weatherArray['almanacPeriod']]['ClockTime'];
		...
		$y = substr($tempdate,6,4);
		$M = substr($tempdate,3,2);
		$d = substr($tempdate,0,2);
		...
		//CALCULATING HOURS OF DAYLIGHT -- OBSELETE WITH WEEWX
		//		$weatherArray['dawnToDuskTime'] = $weatherArray['civilsetTime'] - $weatherArray['civilriseTime'];
		
		
		/*This is a Check that the Weather Station is sending current data.. ie. has it crashed... if the difference between now and than it the offset then we know something is wrong.  The Station is UTC so its going to be ahead of current time for my timezone... you will want to shoot for your configuration*/
